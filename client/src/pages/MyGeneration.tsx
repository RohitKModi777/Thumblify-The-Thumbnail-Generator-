         ...
         {!loading && thumbnails.length > 0 && (
          <div className="columns-1 sm:columns-2 lg:columns-3 2xl: columns-4 gap-8">
             {thumbnails.map((thumb: IThumbnail) => {
              ...
              return (
                 <div key ={thumb._id} onClick={()=>navigate(`/generate/${thumb._id}`)} className="mb-8 group relative cursor-pointer rounded-2xl border border-white/10 transition shadow-xl break-inside-avoid">
                      
                      {/* Imaage */}
                      <div className={`relative overflow-hidden rounded-t-2xl ${aspectClass} bg-black`}>
                        {thumb.image_url ? (
                          <img src={thumb.image_url} alt={thumb.title} className="w-full h-full object-covber group-hover:scale-105 tranisition-transform duaration-300"/>
                        ...
                        {thumb.isGenerating && <div className="absolute inset-0 bg-black/50 flex items-center justiy-center text-sm font-medium text-white">Genrating...</div>}
                      </div>
