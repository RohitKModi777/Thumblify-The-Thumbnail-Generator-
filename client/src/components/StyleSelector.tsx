...
const StyleSelector = ({value,onChange,isOpen,setIsOpen}:{value:ThumbnailStyle; onChange:(style:ThumbnailStyle)=>void;isOpen: boolean;setIsOpen:(open: boolean)=>void}) => {
    ...
    const styleDescriptions: Record<ThumbnailStyle, string>={
        ...
        'Photorealistic':'Ultra-realistic, natural lighting, lifelike details',
    ...
    const styleIcons: Record<ThumbnailStyle, React.ReactNode>={
        ...
        'Illustrated':<ImageIcon className="h-4 w-4"/>,
        'Tech/Futuristic':<CpuIcon className="h-4 w-4"/>
    }

  return (
    <div className="relative space-y-3 dark">
       <label className="block text-sm front-medium text-zinc-200">Thumbnail Style</label>  

       <button type="button" onClick={()=> setIsOpen(!isOpen)} className="flex w-full items-ceter justify-between rounded-md border px-4 py-3 text-left transition bg-white/8 border border-white/10 text-zinc-200 hover:bg-white/12">
         ...
         {thumbnailStyles.map((style)=>{
            return (
              <button
                ...
                className="flex w-full items-center gap-3 px-4 py-3 text-left transition hover:bg-back/30"
              >
